...
**Status:** IN PROGRESS **Introduced:** Scoping document, February 2026 **Statement:** A "good explanation" in Deutsch's sense can be formally characterized as a faithful, essentially surjective functor E: T → P from a minimal source category T (theoretical structure) to a target category P (empirical phenomena). **Lineage:** Direct formalization of Deutsch's qualitative criterion. Core thesis of the project. **If true:** Provides the first formal definition of explanatory goodness. Unlocks everything downstream. **If false:** The entire framework needs rethinking. But _how_ it fails will be informative — does it fail because functors are too rigid, too loose, or the wrong kind of morphism entirely? **Dependencies:** Requires rigorous construction of P and T for at least one physical theory. **Evidence (Session 003):** Ptolemy-vs-Kepler case study. Framework correctly classifies Ptolemy (fails faithfulness — epicycle decomposition is non-unique Fourier decomposition; fails minimality — epicycles freely added/removed) and Kepler (faithful — orbital elements uniquely determine trajectory; essentially surjective; approximately minimal). Verdict is non-trivial: distinguishes "data-fitting" from "explanation" via faithfulness. Analysis is informal — formal construction needed.
